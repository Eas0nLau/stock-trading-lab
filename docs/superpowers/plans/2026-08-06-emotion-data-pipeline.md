...
- `stock_daily`：2026 年 142 个交易日，`20260105` 至 `20260805`，共 779,891 行。
- `t_指数情绪周期_每日分析`：142 个交易日，全年目标范围完整。
- `t_韭研公社异动解析`：目标源日期 143 天，已完成 79 天，剩余 64 天。
- `t_热门板块情绪_每日分析`：77 个交易日，共 1,193 行；只计算前后两个交易日韭研源数据均存在的日期。
- 全套测试：33 passed。

暂停原因：

- 连续访问历史页面后，从 `20260325` 起大量日期返回空数据或超时，疑似韭研当前登录会话触发历史访问限制。
- 为避免账号或 IP 被封，2026-08-06 当天停止继续访问，不伪造空日期数据。

明日续跑：

1. 确认项目 Chrome profile 仍处于登录状态。
2. 先只测试 `20260325` 一个日期；成功后再恢复批量任务。
3. 批量采集仍使用 Redis 跨进程限流，每次请求至少间隔 60 秒，建议每批不超过 10 个交易日。
4. 每批按数据库已有日期断点续跑，禁止重新请求已完成日期。
5. 韭研源补齐后，仅重算缺失的板块情绪日期，并再次检查 `综合状态='数据缺失'` 数量。

当前首批缺失日期从以下序列开始：

```text
20260325, 20260326, 20260327, 20260330, 20260331,
20260401, 20260402, 20260403, 20260407, 20260408
```
