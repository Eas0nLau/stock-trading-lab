...
- 将正式代码中的中文目录、文件名和标识符迁移为英文命名。
- 将新版 API 字段、配置业务键和数据库表列迁移为英文命名。
- 整理研究脚本与生产运行代码的关系。
...
- 不在没有版本化迁移脚本、备份校验和回滚说明的情况下修改数据库表结构。
- 不将所有历史文件一次性移动后再统一修复。
...
- 包、目录、模块和文件使用 `snake_case`。
- 函数、方法和变量使用 `snake_case`。
- 类、异常和数据模型使用 `PascalCase`。
- 常量使用 `UPPER_SNAKE_CASE`。
- Vue 组件使用 `PascalCase`，前端变量和函数使用 `camelCase`。
- 中文只保留在界面文案、日志、领域展示名称和必要的第三方原始数据中。

数据库与 API 也迁移为英文契约：

- 新版接口使用 `/api/v1` 和英文 JSON 字段。
- 旧中文 API 在对应模块完成迁移前由映射层提供有限兼容，完成迁移并更新前端后删除。
- 每张业务表独立提供版本化 SQL 迁移脚本，明确表名、列名、索引、外键和 JSON 字段映射。
- 数据库迁移执行前记录行数、主键范围和关键字段空值数量；执行后用相同指标校验。
- 迁移脚本必须提供回滚 SQL 或明确的备份恢复步骤，禁止仅修改 ORM/SQL 查询而不迁移存量数据。
- Repository 内部只暴露英文领域模型，第三方中文字段在 adapter 边界转换。
- `.env` 环境变量保持大写英文；配置中的中文业务键迁移为英文属性，旧 `config.py` 在兼容期映射旧名称。

数据库迁移按业务模块逐表进行，不在基础设施阶段一次性修改全部表。模块只有在数据迁移、repository、API、前端和测试全部通过后才算完成。
...
- API 覆盖正常响应、空数据和异常响应。
- 兼容测试验证旧入口可以调用新实现。
- 契约测试验证英文 API 字段以及兼容期中文字段映射。
- 数据迁移测试验证迁移前后行数、主键和关键业务值一致，并验证回滚路径。
...
- `docs/database-migrations.md`：表和字段映射、执行顺序、校验与回滚。
...
- 现有功能在迁移后继续可用。
- 文档与实际目录、命令和配置一致。
- 正式代码、API 和数据库契约使用英文命名，中文只存在于明确允许的展示与兼容边界。
