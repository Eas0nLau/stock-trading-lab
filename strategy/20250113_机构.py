...
--         AND 股票代码 = 002757
    ...
    for row in data_list:
        if row['净买入额'] < -1000:
            continue
        if row['日期类型'] != '1日' and row['日期类型'] != '3日':
            continue

        机构买入家数 = 0
        for i in range(1, 6):
            if row[f'买{i}营业部'] == '机构专用' and row[f'买{i}买入额'] > 20000 and row[f'买{i}卖出额'] < 100:
                # logger.warning(f"{row['股票代码']} {row['股票名称']} {row[f'买{i}营业部']}：买入额{row[f'买{i}买入额']} 卖出额{row[f'买{i}卖出额']}")
                机构买入家数 += 1
            # 大于2亿
            # if row[f'买{i}营业部'] == '机构专用' and row[f'买{i}买入额'] > 20000 and row[f'买{i}卖出额'] < 100:
            #     # logger.warning(f"{row['股票代码']} {row['股票名称']} {row[f'买{i}营业部']}：买入额{row[f'买{i}买入额']} 卖出额{row[f'买{i}卖出额']}")
...
if __name__ == "__main__":
    ...
    # main(start_date=20260512, end_date=20260518)
