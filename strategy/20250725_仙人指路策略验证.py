...
def strategy(filtered_codes, target_date):
    ...
    for ts_code in tqdm(filtered_codes):
        ...
        # 20250701
        # if target_row['stock_name'] not in ['上海电影']:
        #     continue
        ...
        if target_row['pct_chg'] > 7:
            ...
            continue
        if target_row['high'] < df.iloc[:-1]['high'].max():
            # logger.info(f"不满足 {range_days}日新高 跳过")
            continue
        if target_row['high'] < target_row['close']:
            continue
        if target_row['high'] <= target_row['open']:
            continue

        recent_data = df[df['trade_date'] < target_date].tail(60)

        if target_row['high'] < recent_data['high'].max():
            continue
        # 上影线偏离值
        volatility = abs(target_row['high'] - target_row['close']) / min(target_row['high'], target_row['close']) * 100
        if volatility < 3:
            continue
        max_vol = df['amount'].mean()
        if target_row['amount'] <= max_vol * 1.2:
            # logger.info(f"不满足 计算当天成交交是否大于 前{range_days}日最大成交额*{1.2} 跳过")
            continue
