...
def strategy(filtered_codes, target_date):
    ...
    for row in data_list:
        if row['code'][2] == '3' or row['code'][2:5] == '688':
            continue
        if row['板块'] in 板块_top and '09:30:00' not in str(板块_top[row['板块']]['涨停时间']):
            continue
        板块_top[row['板块']] =row
    ...
    if not filtered_codes:
        return pd.DataFrame([])
    # 加载日线数据
    start_date = (datetime.strptime(str(target_date), "%Y%m%d") - timedelta(days=10)).strftime(
        '%Y%m%d')  # 余量确保足够数据
    ...
    for ts_code in filtered_codes:
        ...
        if len(target_data) < 3:  # 需要足够数据计算均量
            continue
        # if target_data[-3:]['pct_chg'].sum() > 20:
        #     logger.warning(f"{target_data.iloc[0]['stock_name']} 近3个交易日涨幅累计>20% 不接")
        #     continue
...
def simulated_buy():
    ...
    for stock_name in stock_name_list:
        ...
        if volatility < 0:
            ...
            continue
        if volatility > 9.96:
            logger.error(f"{stock_name} 开盘跟昨日收盘偏离:{volatility} 开太高不买")
            continue
...
def simulated_sell(sell_out_fall_threshold=None,
    ...
    if selected_stocks:
        ...
        for ts_code in selected_stocks:
            ...
            #     continue
            # if stock_info['持股天数'] < 2 and _持仓最高回撤 < -5:
            #     account.sell(stock_info['name'], ts_code, stock_now_date_df['close'], stock_info['lots'], now_date)
            #     continue
            ...
            #     logger.info(f"{stock_info['name']} 首次盈亏比> 7% 减半仓")
...
if __name__ == "__main__":
    filename = __file__.split(".py")[0].split("\\")[-1].split("/")[-1].split("/")[-1]
    # logger.add(f'../logs/{filename}.log', rotation="1 MB", encoding='utf-8', retention='1 h')
    main(start_date=20200630, end_date=20211019)
    # main(start_date=20200101, end_date=20270101)
