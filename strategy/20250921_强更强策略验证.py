...
def strategy(filtered_codes, target_date):
    ...
    for ts_code in tqdm(filtered_codes):
        ...
        # 20250701
        # if target_row['stock_name'] not in ['德明利']:
        #     continue
        ...
        if target_row['close'] < df.iloc[:-1]['high'].max():
            ...
            continue
        volatility = (target_row['close'] - df.iloc[-4]['open']) / df.iloc[-4]['open'] * 100
        logger.info(f"近3日涨幅：{volatility}")
        if volatility < 23:
            continue
        pct_chg_mean = df.iloc[-4:-1]['pct_chg'].mean()
        logger.info(f"前2日平均涨幅：{pct_chg_mean}")
        if pct_chg_mean > 9:
            continue
        if pct_chg_mean < 4:
            continue
