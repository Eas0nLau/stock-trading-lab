...
def strategy(filtered_codes, target_date):
    ...
    # ==================== 纯 NumPy 核心 ====================
    df = 日线数据.sort_values(['ts_code', 'trade_date'])  # 确保每只股票按日期排序

    # 转换为 NumPy 数组（只取必要列）
    ts_codes = df['ts_code'].to_numpy()
    stock_names = df['stock_name'].to_numpy()
    closes = df['close'].to_numpy()
    pct_chgs = df['pct_chg'].to_numpy()
    ...
    for code in unique_codes:
        ...
        # 提取当前股票的所有数据
        code_name = stock_names[mask][-1]
        code_close = closes[mask]
        code_pct = pct_chgs[mask]
        code_vol = vols[mask]
        code_open = opens[mask]
        code_high = highs[mask]
        code_low = lows[mask]

        # ========== 所有条件判断（纯 NumPy） ==========
        if len(str(code)) > 5 and str(code)[:2] in ['92', '68', '30']:
            continue
        if 'ST' in str(code_name):
            continue
        if code_pct[-1] < 0 or code_pct[-1] > 5:
            continue
        if code_close[-1] > 200:
            continue
        if code_pct.min() < -9.95:
            continue

        # 一字板判断
        一字板 = np.any((code_open == code_high) & (code_high == code_low) & (code_low == code_close))
        if 一字板:
            continue

        # 近5日累计跌幅
        近5日累计 = code_pct[-6:-1].sum()
        if 近5日累计 > -8:
            continue
        ...
        if n < 11:
            continue
        前10日均量 = code_vol[-11:-1].mean()
        量比 = code_vol[-1] / 前10日均量
        if 量比 < 1.5:
            continue

        候选列表.append({
            'ts_code': common.normalize_symbol(code),
            'stock_name': code_name,
            'trade_date': target_date,
            'close': float(code_close[-1]),
            'pct_chg': float(code_pct[-1]),
            '近5日累计跌幅': float(近5日累计),
            '量比': float(量比)
        })
    ...
    logger.warning(f"入选股票：{' '.join([x['stock_name'] for x in 最终前3只])}")
