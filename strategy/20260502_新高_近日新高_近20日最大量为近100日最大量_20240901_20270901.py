...
def process_stock_batch(args):
    ...
    近2年 =(datetime.strptime(str(target_date), "%Y%m%d") - timedelta(days=720)).strftime(
        '%Y%m%d')
    results = []
    for ts_code in codes_batch:
        ...
        # 20250701
        # if target_row['stock_name'] not in ['中国重汽']:
...
if __name__ == "__main__":
    main(start_date=20260401, end_date=20270901)
