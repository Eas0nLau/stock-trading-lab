...
def strategy(filtered_codes, target_date):
    ...
    for ts_code in tqdm(filtered_codes):
        ...
        # 20250701
        # if target_row['stock_name'] not in ['沙河股份']:
        #     continue
        ...
        if len(df) < min_rise_days:
            continue
        if target_row['vol'] < df.iloc[-2]['vol']:
            continue
        if target_row['close'] < df.iloc[-2]['open']:
            continue
        if target_row['close'] < df.iloc[-2]['close']:
            continue
        first_vol = df.iloc[-2]['vol']
        first_close = df.iloc[-2]['close']
        ...
        for i in range(-1, -min_rise_days - 1, -1):
            try:
                if first_vol <= recent_data['vol'].iloc[i] and first_close <= recent_data['close'].iloc[i]:
                    first_vol = recent_data['vol'].iloc[i]
                    first_close = recent_data['close'].iloc[i]
                    continue
...
def simulated_sell(sell_out_fall_threshold=None,
    ...
    if selected_stocks:
        ...
        for ts_code in selected_stocks:
            ...
            if _持仓最高回撤 < sell_out_盈利回撤_threshold:
                logger.error(
