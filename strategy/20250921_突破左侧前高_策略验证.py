...
def strategy(filtered_codes, target_date):
    ...
    for ts_code in tqdm(filtered_codes):
        ...
        # 20250701
        # if target_row['stock_name'] not in ['盛新锂能']:
        #     continue
        ...
        if target_row['close'] < df.iloc[:-1]['high'].max():
            ...
            continue
        if df.iloc[-2]['close'] > df.iloc[:-2]['high'].max():
            # 已经突破新高
            continue

        if df.iloc[-2]['high'] > df.iloc[:-2]['high'].max():
            # 已经突破新高
            continue
        ...
        if 上一次最高价距离天数 < 20:
            continue
        # 成交量突破
        最近五日成交量 = df.iloc[-5:]['vol'].mean()
        上次新高五日成交量 = df.iloc[row_index - 4:row_index + 1]['vol'].mean()
        logger.warning(f"最近五日成交量:{最近五日成交量} 上次新高五日成交量:{上次新高五日成交量}")
        if 最近五日成交量 < 上次新高五日成交量:
            # 未放量
            continue
...
def main():
    ...
    distinct_trade_date = db.mysql_localhost(sql=f"""
        ...
        where trade_date >= '20260428'
        and trade_date < '20291001'
        order by trade_date
    """, fetch=True)
