...
def process_stock_batch(args):
    ...
    for ts_code in codes_batch:
        ...
        # 20250701
        # if target_row['stock_name'] not in ['坤泰股份']:
        ...
        # 检查当天是否上涨
        if target_row['pct_chg'] > 7:
            continue
        # 近3日有过涨停
        if df.tail(3)['pct_chg'].max() > 9.5:
        ...
        if len(df) < range_days:  # 需要足够数据计算均量
            continue
        if target_row['high'] < df.tail(250).iloc[:-1]['high'].max():
            # logger.info(f"不满足 {60}日新高 跳过")
            continue
        if target_row['close'] < df.tail(250).iloc[:-1]['high'].max():
            # logger.info(f"不满足 {60}日新高 跳过")
            continue
        # 上一天还不是新高
        # if df.tail(2).iloc[-2]['high'] > df.tail(180).iloc[:-2]['high'].max():
        #     # logger.info(f"不满足 {60}日新高 跳过")
        #     continue
        # 上一次新高不在30天内
        if df.tail(30).iloc[:-1]['high'].max() >= df.tail(180).iloc[:-1]['high'].max():
            # logger.info(f"不满足 {60}日新高 跳过")
            continue
        # 连续放量 昨天的量要比今天小
        if df.tail(30).iloc[-1]['amount'] < df.tail(30).iloc[-2]['amount']:
            continue
        # 连续放量 前天的量要比昨天小
        if df.tail(30).iloc[-2]['amount'] < df.tail(30).iloc[-3]['amount']:
            continue
        # 连续放量 前天的量要比昨天小
        if df.tail(30).iloc[-3]['amount'] < df.tail(30).iloc[-4]['amount']:
            continue
        # 连续涨
        if df.tail(30).iloc[-1]['close'] < df.tail(30).iloc[-2]['close']:
            continue
        # 连续涨
        if df.tail(30).iloc[-2]['close'] < df.tail(30).iloc[-3]['close']:
            continue
        # 连续涨
        if df.tail(30).iloc[-3]['close'] < df.tail(30).iloc[-4]['close']:
            continue
